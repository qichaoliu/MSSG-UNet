...
class GCNLayer_PyG(nn.Module):
    def __init__(self, input_dim: int, output_dim: int, adjacency_matrix:torch.Tensor, position: torch.Tensor,neighbors:int):
        ...
        # 第一层GCN
        self.GCN_liner_out_1 = nn.Sequential(nn.Linear(input_dim, output_dim))#,nn.LeakyReLU(inplace=True)
        self.GCN_liner_theta_1 = nn.Sequential(nn.Linear(input_dim, 128),nn.LeakyReLU(inplace=True))#,nn.ReLU()
        ...
        if self.neighbors>0:
            ...
            self.col, self.row = self.edge_index = knn_graph(self.position, self.neighbors, batch=None, loop=True)  # row已经排好序 [0,node_count*4) 升序
        else:
            # unfixed neighbors
    ...
    def forward(self, H, prior_A=None):
        ...
        if self.neighbors>0:
            ...
            A = torch.where(self.mask > 0, e, zero_vec)*(1-self.I)+ self.lambda_*self.I
...
class HiGCN(nn.Module):
    def __init__(self, height: int, width: int, changel: int, class_count: int,
        ...
        self.layer_count=len(hierarchy_matrices) # 有多少图卷积层-pooling层
        self.neighbors = neighbors
        self.S_list_Hat_T = []
        for i in range(len(hierarchy_matrices)):
            temp=hierarchy_matrices[i]
            self.S_list_Hat_T.append((temp/ (torch.sum(temp, 0, keepdim=True,dtype=torch.float32))).t())  # 列归一化Q
        
        ## 计算每层的所有节点的坐标,用于构建KNN
        positions = self.getPositions() # 每个节点的坐标
        self.positions=[]
        for i in range(len(hierarchy_matrices)):
        ...
        for i in range(self.layer_count-1):
            # self.De_GCN_layers.add_module('my_DeGCN_layer'+str(i), GCNLayer(2*layer_channels,layer_channels,
            #                                                                 self.S_list[self.layer_count-i-1],
            #                                                                 self.A_list[self.layer_count-i-2]))
    ...
    def forward(self, x: torch.Tensor, showFlag=False):
        ...
        for i in range(len(self.GCN_layers)):
            ...
            # H_i = self.GCN_layers[i](H_i,prior_A=edgeMapList[i]) ################
            H_i,A_i = self.GCN_layers[i](H_i,prior_A=None) ################
            encoder_features.append(H_i)
            A_encoders.append(A_i)
            
        A_decoders.append(A_encoders[-1])# decoder 与encoder 最后一层 为同一个
        decoder_features.append(H_i)
        
        # for experiment 阻断某个skip connection #####################################
        # encoder_idx=0
        # encoder_features[encoder_idx]=encoder_features[encoder_idx]*0
        # encoder_features[encoder_idx]=encoder_features[encoder_idx].detach()
        # decoder GCN
        for i in range(len(self.De_GCN_layers)):
            H_i=torch.mm(self.S_list[-i-1], H_i)
            H_i=torch.cat([H_i,encoder_features[-i-2]],dim=-1)  # concat 模式
            # H_i=H_i+encoder_features[-i-2]  # add 模式
            # H_i = self.De_GCN_layers[i]( H_i ,prior_A=edgeMapList[-i-2]) ################
